...
def chart_hrv_bb(df: pd.DataFrame, anomaly_dates: list | None = None) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=df["bb_highest"],
            name="身体电量",
            line=dict(color=COLOR_BB, width=2),
        ),
        secondary_y=True,
    )
...
def chart_bp(df_vitals: pd.DataFrame) -> go.Figure:
    ...
    if not df_vitals.empty:
        fig.add_hrect(
            y0=0,
            y1=120,
            fillcolor=REFERENCE_BAND_COLOR,
            line_width=0,
            annotation_text="正常收缩压",
            annotation_position="top left",
        )
        fig.add_hrect(y0=0, y1=80, fillcolor="rgba(100,100,200,0.05)", line_width=0)
    ...
    if not df_bp.empty:
        fig.add_trace(
            go.Scatter(
                ...
                y=df_bp["systolic"],
                name="收缩压",
                line=dict(color=COLOR_SYSTOLIC, width=2),
                mode="lines+markers",
            )
        )
        fig.add_trace(
            go.Scatter(
                ...
                y=df_bp["diastolic"],
                name="舒张压",
                line=dict(color=COLOR_DIASTOLIC, width=2),
                mode="lines+markers",
            )
        )
    if "heart_rate" in df_vitals.columns and df_vitals["heart_rate"].notna().any():
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=df_hr["heart_rate"],
                name="脉率",
                line=dict(color=COLOR_PULSE, width=1.5, dash="dot"),
                mode="lines+markers",
            )
        )

    fig.update_layout(title="血压趋势", hovermode="x unified", height=320, yaxis_title="mmHg")
    return fig
...
def chart_weight_fat(df_vitals: pd.DataFrame, title: str = "体重 & 体脂率趋势") -> go.Figure:
    ...
    if not df_weight.empty:
        fig.add_trace(
            go.Scatter(
                x=df_weight["measured_at"],
                y=df_weight["weight_kg"],
                name="体重",
                line=dict(color=COLOR_WEIGHT, width=2),
                mode="lines+markers",
            )
        )
    ...
    if not df_fat.empty:
        fig.add_trace(
            go.Scatter(
                x=df_fat["measured_at"],
                y=df_fat["body_fat_pct"],
                name="体脂率",
                line=dict(color=COLOR_FAT, width=2),
                yaxis="y2",
                mode="lines+markers",
            )
        )
...
def chart_stress(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=df["stress_average"],
            name="压力指数",
            line=dict(color=COLOR_STRESS, width=2),
            fill="tozeroy",
            fillcolor="rgba(200,117,209,0.1)",
        )
    )
    _add_7day_avg(fig, df, "stress_average", "压力", COLOR_STRESS)

    fig.update_layout(
        title="压力指数趋势", hovermode="x unified", height=320, yaxis_title="压力指数"
    )
    return fig
...
def chart_trend_prediction(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=list(pred_values),
            name="预测",
            line=dict(color="#F7A84F", width=2, dash="dash"),
        )
    )
    fig.add_trace(
        go.Scatter(
            ...
            fill="toself",
            fillcolor="rgba(247,168,79,0.15)",
    ...
    fig.update_layout(title=title, yaxis_title=unit, hovermode="x unified", height=320)
    return fig
