...
from superhealth.dashboard.data_loader import get_upcoming_appointments, load_lab_results, load_eye_exams


def _render_risk_card(result: dict, title: str,
...
def render():
    ...
    with tab2:
        ...
        if sbp or dbp:
            ...
            st.caption(
                f"最新血压：{sbp_str}/{dbp_str} mmHg "
                f"（分级：{ht_result.get('bp_grade', '未知')}）"
            )
    ...
    with tab3:
        ...
        if ldl is not None:
            color = "green" if status == "达标" else "orange"
            st.caption(
                f"LDL-C：{ldl:.2f} mmol/L（目标 <{target}，{status}）"
            )
    ...
    with tab5:
        ...
        for key, title, unit in configs:
            ...
            fig = chart_trend_prediction(
                p["hist_dates"], p["hist_values"],
                p["pred_dates"], p["pred_values"],
                p["pred_upper"], p["pred_lower"],
    ...
    with tab6:
        ...
        ua_df = df_lab[df_lab["item_name"].str.contains(
            "尿酸|Uric Acid|UA", case=False, na=False)]
        if not ua_df.empty:
            latest_ua = ua_df["value"].iloc[-1]
            last_ua_date = ua_df["date"].iloc[-1].date() if hasattr(ua_df["date"].iloc[-1], "date") else ua_df["date"].iloc[-1]
        ...
        if not df_eye.empty:
            ...
            last_eye_date = latest_eye["date"].date() if hasattr(latest_eye["date"], "date") else latest_eye["date"]
            days_since_eye = (date.today() - last_eye_date).days
            iop_left = latest_eye.get("iop_left")
            iop_right = latest_eye.get("iop_right")
            iop_max = max(v for v in [iop_left, iop_right] if v is not None) if any(
                v is not None for v in [iop_left, iop_right]
            ) else None
