...
GOAL_INTERVENTIONS: dict[str, list[tuple[str, str, int]]] = {
    "bp_systolic_mean_7d": [
        ("等长运动强化", "每日等长握力训练 4 组 × 3 分钟（30% MVC），组间休息 2 分钟；每周递增 10 秒（中国高血压防治指南 2024）", 14),
        ("哈他瑜伽干预", "每周 5 次 45 分钟哈他瑜伽，含站立/平衡/倒箭式（AHA 证据：收缩压降 5-8 mmHg）", 21),
        ("杨式太极拳干预", "每日 40 分钟杨式太极拳 24 式全套练习（Meta 分析：收缩压降 8-12 mmHg）", 21),
        ("中等强度有氧累积", "每日累积 40 分钟快走（心率 110-130 bpm），可分 2 次完成，每周 5 天以上（AHA 推荐量 150 min/周）", 14),
        ("靠墙静蹲+平板支撑组合", "每日 3 组靠墙静蹲（力竭）+ 3 组平板支撑（60 秒），组间休息 90 秒（等长抗阻降压证据最强）", 14),
    ],
    "bp_diastolic_mean_7d": [
        ("等长运动强化", "每日等长握力训练 4 组 × 3 分钟（30% MVC），组间休息 2 分钟", 14),
        ("共振呼吸干预", "每日 2 次 15 分钟共振呼吸（5.5 bpm），晨起+睡前各一次", 14),
        ("有氧规律化干预", "每日 40 分钟中等强度有氧（快走/慢跑/骑车），固定时段，每周 5 天以上", 21),
        ("靠墙静蹲+深蹲组合", "每日 4 组靠墙静蹲（力竭）+ 4 组徒手深蹲（15 次），组间休息 90 秒", 14),
        ("渐进式肌肉放松", "每日睡前 20 分钟 Jacobson 渐进式肌肉放松（16 组肌群依次紧张-放松）", 14),
    ],
    "hrv_mean_7d": [
        ("晨间有氧干预", "每日晨间 35 分钟中等强度有氧（心率 120-140 bpm），提升副交感神经张力", 14),
        ("4-7-8 呼吸干预", "每日 3 次 4-7-8 呼吸法（吸气 4s-屏息 7s-呼气 8s），每次 8 个循环", 14),
        ("瑜伽恢复干预", "每周 4 次 45 分钟流瑜伽/阴瑜伽交替，低 HRV 日仅做阴瑜伽（前屈+扭转为主）", 14),
        ("冷水暴露干预", "每日淋浴末 30 秒冷水（15-20°C），逐渐延长至 60 秒（激活迷走神经，Meta 分析 HRV 提升 15%）", 14),
        ("太极+冥想组合", "每日 20 分钟太极拳 + 10 分钟正念冥想（呼吸锚定），晨起执行", 14),
    ],
    "resting_hr_mean_7d": [
        ("有氧规律化干预", "每日 40 分钟有氧运动（快走/慢跑/游泳），固定晨间时段，每周 5 天以上", 21),
        ("睡眠优化干预", "固定 23:00 前入睡，睡前 90 分钟无蓝光屏幕，卧室温度 18-20°C，确保 7.5 小时以上", 14),
        ("呼吸训练干预", "每日 2 次箱式呼吸（吸气 4s-屏 4s-呼 4s-屏 4s），每次 10 分钟，降低交感张力", 14),
        ("高强度间歇训练", "每周 3 次 25 分钟 HIIT（30s 全力 + 60s 恢复 × 15 组），提升心肺效率降低静息心率", 14),
        ("瑜伽 nidra 引导", "每日睡前 20 分钟瑜伽 nidra（身体扫描引导放松），降低皮质醇和静息心率", 14),
    ],
    "sleep_score_mean_7d": [
        ("严格规律作息", "固定 23:00 入睡 + 6:30 起床，周末偏差不超过 15 分钟，包含节假日", 14),
        ("睡前 90 分钟仪式", "睡前 90 分钟：停止工作+蓝光 → 30 分钟拉伸 → 15 分钟 4-7-8 呼吸 → 15 分钟阅读（纸质书）", 14),
        ("晨间光照干预", "起床后 30 分钟内接受 15-20 分钟户外自然光（或 10000 lux 光疗灯），稳固昼夜节律", 14),
        ("午后禁咖啡因", "下午 2 点后完全禁咖啡/茶/可乐，改喝菊花茶或温水（咖啡因半衰期 5-6 小时干扰深睡）", 14),
        ("睡前体温干预", "睡前 90 分钟热水浴（40-42.5°C，10-15 分钟），核心体温先升后降促进入睡", 14),
    ],
    "stress_mean_7d": [
        ("共振呼吸干预", "每日 3 次 10 分钟共振呼吸（5.5 bpm），晨起/午休/睡前，配合心率变异性生物反馈", 14),
        ("正念冥想渐进", "每日 20 分钟正念冥想（身体扫描→呼吸锚定→开放觉察），从 10 分钟递增至 20 分钟", 14),
        ("自然暴露干预", "每周 5 次户外自然环境活动 30 分钟以上（森林浴/公园散步），远离手机", 14),
        ("运动减压组合", "每日 30 分钟中等强度有氧（心率 120-140 bpm）+ 拉伸 10 分钟，运动后内啡肽持续减压", 14),
        ("日记书写干预", "每日睡前 15 分钟表达性书写（写下今日压力源+应对策略+感恩 3 件事），Meta 分析减压效果 d=0.5", 14),
    ],
    "weight_kg_mean_7d": [
        ("空腹有氧干预", "每周 4 次晨起空腹快走 40-50 分钟（心率 110-125 bpm），加速脂肪氧化", 21),
        ("抗阻训练干预", "每周 4 次全身力量训练（推拉腿分化），每次 50 分钟，组间休息 60-90 秒", 21),
        ("16:8 间歇断食", "每日进食窗口压缩至 8 小时（如 10:00-18:00），其余 16 小时仅饮水/黑咖啡", 21),
    ],
    "body_fat_pct_mean_7d": [
        ("HIIT 干预", "每周 4 次 25 分钟高强度间歇（30s 全力 + 60s 恢复 × 15 组），运动后过量氧耗持续燃脂", 21),
        ("抗阻训练干预", "每周 4 次全身力量训练（大肌群复合动作为主），每次 50 分钟，追求渐进超负荷", 21),
        ("组合训练干预", "每周 3 次力量（40 分钟）+ 2 次有氧（35 分钟中等强度），力量日优先", 21),
    ],
    "body_battery_wake_mean_7d": [
        ("睡眠优化干预", "固定 23:00 前入睡，睡前 90 分钟无蓝光屏幕，确保 7.5-8 小时高质量睡眠", 14),
        ("晨间恢复仪式", "起床后：5 分钟深呼吸 + 10 分钟动态拉伸 + 10 分钟低强度散步，启动 Body Battery 充电", 14),
        ("日间微恢复", "每工作 90 分钟起身 5 分钟（深呼吸+拉伸+走动），防止 Body Battery 非必要耗尽", 14),
    ],
    "steps_mean_7d": [
        ("阶梯递增干预", "每日步数在当前基础上增加 3000 步，分 2-3 次完成（如晨间+午休+晚餐后）", 14),
...
class ExperimentManager:
    ...
    def suggest_for_goal(self, goal_id: int, force_regenerate: bool = False) -> list[dict]:
        ...
        dir_label = {"decrease": "降低", "increase": "提升", "stabilize": "稳定"}.get(direction, direction)
        result = []
    ...
    def _generate_with_llm(self, goal_id: int, metric_key: str,
                            direction: str, goal_name: str) -> list[dict] | None:
        """调用百川生成干预方案。"""
        try:
            ...
            try:
                ...
                contraindications = [c for c in profile.exercise_contraindications] if profile.exercise_contraindications else []
            except Exception:
                pass
            ...
            if contraindications:
                contra_labels = {"valsalva_maneuver": "憋气/Valsalva 动作", "inverted_positions": "倒立体位", "maximal_isometric": "最大等长收缩"}
        ...
        with self._get_conn() as conn:
            ...
            cursor = conn.execute(
                """INSERT INTO experiments
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, 'draft', ?, ?)""",
                (name, hypothesis, goal_id, metric_key, direction,
                 intervention, min_duration, notes),
            )
    ...
    def activate(self, experiment_id: int) -> None:
        ...
        log.info(
            ...
            experiment_id, start, end, exp["metric_key"],
        )
    ...
    def _evaluate_with_causal(self, exp: dict, causal_key: str,
                               start_date: str, period_days: int) -> dict:
        ...
        paired = analyzer.paired_intervention_test(
            causal_key, start_date, period_days=period_days
        )
        itsa = analyzer.interrupted_time_series(
            causal_key, start_date, pre_days=14, post_days=period_days
        )

        direction_match = self._direction_matches(exp["direction"], paired.difference, paired.baseline_mean)
    ...
    def _evaluate_from_goal_progress(self, exp: dict) -> dict:
        ...
        if not goal_id:
            return {"verdict": "revert", "commit": False, "inconclusive": False,
        ...
        if b_mean is None or p_mean is None or n < 7:
            return {"verdict": "inconclusive", "commit": False, "inconclusive": True,
    ...
    def _direction_matches(self, expected_direction: str, diff: float, baseline: float = None) -> bool:
    ...
    def _commit(self, exp: dict, result: dict) -> None:
        ...
        d_label = {"decrease": "降低", "increase": "提升", "stabilize": "稳定"}.get(exp["direction"], exp["direction"])
