...
def load_day(day_str: str) -> dict[str, Any] | None:
    ...
    for md_path in [DATA_DIR / f'{day_str}.md',
...
def score_state(cur: dict[str, Any], baselines: dict[str, dict] | None = None) -> tuple[int, str, list[str]]:
    score = 0
    notes = []
    bl = baselines or {}
    ...
    if st is not None:
        if st_z is not None:
            if st_z >= 1.0:
                score += 2; notes.append(f'睡眠评分较好（高于个人基线{st_z:.1f}σ）')
            elif st_z >= 0.5:
                score += 1; notes.append(f'睡眠评分尚可（高于个人基线{st_z:.1f}σ）')
            else:
                score -= 1; notes.append(f'睡眠恢复一般（低于个人基线{abs(st_z):.1f}σ）')
        else:
            if st >= 85:
                score += 2; notes.append('睡眠评分较好')
            elif st >= 75:
                score += 1; notes.append('睡眠评分尚可')
            else:
                score -= 1; notes.append('睡眠恢复一般')
    ...
    if bb is not None:
        if bb_z is not None:
            if bb_z >= 1.0:
                score += 2; notes.append(f'起床 Body Battery 较高（高于个人基线{bb_z:.1f}σ）')
            elif bb_z >= 0.5:
                score += 1; notes.append(f'起床 Body Battery 中等（高于个人基线{bb_z:.1f}σ）')
            elif bb_z <= -1.0:
                score -= 2; notes.append(f'起床 Body Battery 偏低（低于个人基线{abs(bb_z):.1f}σ）')
            else:
                score -= 1; notes.append(f'起床 Body Battery 偏弱（低于个人基线{abs(bb_z):.1f}σ）')
        else:
            if bb >= 75:
                score += 2; notes.append('起床 Body Battery 较高')
            elif bb >= 55:
                score += 1; notes.append('起床 Body Battery 中等')
            elif bb < 40:
                score -= 2; notes.append('起床 Body Battery 偏低')
            else:
                score -= 1; notes.append('起床 Body Battery 偏弱')

    # ── HRV ──
    hrv_status = (cur.get('hrv_status') or '').upper()
    if hrv_status == 'BALANCED':
        score += 2; notes.append('HRV 状态平衡')
    elif hrv_status == 'UNBALANCED':
        notes.append('HRV 有所波动')
    elif hrv_status == 'LOW':
        score -= 2; notes.append('HRV 偏低')

    hrv_avg = cur.get('hrv_avg')
    hrv_low = cur.get('hrv_baseline_low')
    if hrv_avg is not None and hrv_low is not None:
        if hrv_avg >= hrv_low:
            score += 1; notes.append('HRV 回到或接近基线')
        else:
            score -= 1; notes.append('HRV 低于个人基线')
    ...
    if rhr is not None:
        if rhr_z is not None:
            if rhr_z <= 0:
                score += 1; notes.append(f'静息心率未高于个人基线（{rhr_z:+.1f}σ）')
            elif rhr_z >= 1.0:
                score -= 1; notes.append(f'静息心率偏高（高于个人基线{rhr_z:.1f}σ）')
        else:
            rhr7 = cur.get('avg7_resting_hr')
            if rhr7 is not None:
                if rhr <= rhr7:
                    score += 1; notes.append('静息心率未高于近期平均')
                elif rhr >= rhr7 + 3:
                    score -= 1; notes.append('静息心率偏高')
    ...
    if stress is not None:
        if stress_z is not None:
            if stress_z <= -0.5:
                score += 1; notes.append(f'平均压力不高（低于个人基线{abs(stress_z):.1f}σ）')
            elif stress_z >= 1.0:
                score -= 1; notes.append(f'平均压力偏高（高于个人基线{stress_z:.1f}σ）')
        else:
            if stress < 25:
                score += 1; notes.append('平均压力不高')
            elif stress > 35:
                score -= 1; notes.append('平均压力偏高')

    # ── 呼吸 ──
    resp = cur.get('resp_waking')
    if resp is not None:
        if resp <= 15:
            score += 1; notes.append('呼吸频率平稳')
        elif resp >= 18:
            score -= 1; notes.append('呼吸频率偏高')
...
def has_meaningful_data(data: dict[str, Any]) -> bool:
    ...
    return (data.get('sleep_score') is not None
...
def main() -> None:
    ...
    lines.append('')
    lines.append('## 今日关键指标')
    lines.append(f"- 睡眠：{fmt_minutes_hm(current.get('sleep_total_min'))} | 睡眠分数 {fmt_val(current.get('sleep_score'))}")
    lines.append(f"- 起床 Body Battery：{fmt_val(current.get('body_battery_wake'))}")
    lines.append(f"- HRV：{fmt_val(current.get('hrv_avg'), ' ms')} | 状态 {fmt_val(current.get('hrv_status'))}")
    lines.append(f"- 静息心率：{fmt_val(current.get('resting_hr'), ' bpm')} | 7天均值 {fmt_val(current.get('avg7_resting_hr'), ' bpm')}")
    ...
    if prev:
        lines.append('## 与前一天对比')
        lines.append(f"- 睡眠分数：今天 {fmt_val(current.get('sleep_score'))} vs 昨天 {fmt_val(prev.get('sleep_score'))}（{fmt_delta(current.get('sleep_score'), prev.get('sleep_score'))}）")
        lines.append(f"- 起床 Body Battery：今天 {fmt_val(current.get('body_battery_wake'))} vs 昨天 {fmt_val(prev.get('body_battery_wake'))}（{fmt_delta(current.get('body_battery_wake'), prev.get('body_battery_wake'))}）")
        lines.append(f"- HRV：今天 {fmt_val(current.get('hrv_avg'))} vs 昨天 {fmt_val(prev.get('hrv_avg'))}（{fmt_delta(current.get('hrv_avg'), prev.get('hrv_avg'), ' ms')}）")
        lines.append(f"- 静息心率：今天 {fmt_val(current.get('resting_hr'))} vs 昨天 {fmt_val(prev.get('resting_hr'))}（{fmt_delta(current.get('resting_hr'), prev.get('resting_hr'), ' bpm')}）")
        lines.append(f"- 平均压力：今天 {fmt_val(current.get('avg_stress'))} vs 昨天 {fmt_val(prev.get('avg_stress'))}（{fmt_delta(current.get('avg_stress'), prev.get('avg_stress'))}）")
        lines.append('')
    ...
    if prev3:
        ...
        lines.append(f"- 近3日平均睡眠分数：{fmt_val(avg_sleep_score)}；今天 {fmt_val(current.get('sleep_score'))}")
        lines.append(f"- 近3日起床 Body Battery：{fmt_val(avg_bb)}；今天 {fmt_val(current.get('body_battery_wake'))}")
        lines.append(f"- 近3日 HRV：{fmt_val(avg_hrv)} ms；今天 {fmt_val(current.get('hrv_avg'))} ms")
        lines.append('')
