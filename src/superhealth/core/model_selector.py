...
class ModelSelector:
    ...
    def select(self, profile: "HealthProfile", daily_data: dict = None,
               goals: list[dict] = None) -> list[ModelSpec]:
        """返回应激活的模型列表（总是包含基础模型）。
        ...
        # 血脂模型：有血脂异常史或 LDL 偏高
        if ("dyslipidemia_history" in profile.history_conditions
        ...
        if bc.weight_kg is not None:
            if bc.bmi_status in ("underweight", "overweight", "obese") or bc.body_fat_status in ("high", "very_high", "low"):
