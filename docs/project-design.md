...
个人健康数据管理系统，整合 Garmin 手表、欧姆龙血压计、华为体脂秤等多源数据，实现自动化采集、长期趋势分析、因果推断和个性化健康洞察。
...
配置文件 `~/.healthy/config.toml`（权限 0o600），环境变量覆盖。
...
Streamlit 应用，入口 `streamlit run src/healthy/dashboard/app.py`，端口 8501。
...
```bash
# /etc/cron.d/healthy-garmin
0 7 * * * root /root/healthy/scripts/run_garmin_daily.sh >> /root/healthy/logs/fetch_garmin_daily.log 2>&1
```
...
- 配置文件 `~/.healthy/config.toml` 权限 0o600，包含所有 API 密钥
