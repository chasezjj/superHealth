...
| `goals` | 阶段性目标（名称/指标/方向/基线/目标值/状态） | CLI 手动设定 |
...
- **目标绑定**：偏好写入时带 `goal_id`，支持按目标查询最佳运动
...
| LLMAdvisor | System Prompt 注入 `### 当前阶段性目标` 段落 + 目标约束指令 |
...
python -m superhealth.goals add --name "降血压" \
